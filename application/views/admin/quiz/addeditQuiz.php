...
<script type="text/javascript">
  ...
  //Get Sub Categories on change of Categories
    $('#catid').change(function() {
   	...
   	if(catid=="") {
   		$('#subcatid').empty();
   		$('#subcatid').append("<option value=''>Select Category First.</option>");
   	}
   	...
   	$.ajax({
   		...
   		success: function(data) {
   			...
   			if (dta.length>0) {
   				$('#subcatid').empty();
   				$('#subcatid').append("<option value=''>Select Sub Category</option>");
   				for(i=0; i<dta.length; i++)
   				$('#subcatid').append("<option value='"+dta[i].subcatid+"'>"+dta[i].name+"</option>");
   			}
   			else {
   				$('#subcatid').empty();
   				alert("No sub categories Available");	
   			}		
   		}
   	});
    });
   ...
   $('#quiz_questions').click(function() {
   	...
   	if($('#subjectid').val()!='' && noOfQuestions!='' && 
   		...
   		if(noOfAvailableQuestions!=0 && noOfQuestions<=noOfAvailableQuestions) {
   			...
   			alert('No questions available in this subject.');
   		}
   		else {
   			alert('Please add the no. of questions within the available range.');
   		}
   	}
   		...
   		alert("Please add minimum 1 question from each subject.");
   	}		
   });
   ...
   if (subjectId!="") {			
   	$.ajax({
   		...
   		success: function(data) {
   			if(data != 0) {						
   				$('#no_of_available_questions_p').append("<i class='fa fa-info-circle'></i> No. of available questions in this subject: "+data);						
   			}
   			else {
   				$('#no_of_available_questions_p').append("<i class='fa fa-info-circle'></i> No questions available in this subject.");
   			}
...
<div class="col-md-12 padd">
   <div class="bradcome-menu">
      <ul>
         ...
         <li><img  src="<?php echo base_url();?>assets/designs/images/arrow.png"></li>
         <li><a href="#"> <?php if(count($data)>0) echo "Update Quiz"; else echo "Add Quiz";?></a></li>
      </ul>
   </div>
</div>
   ...
   <div class="col-md-8">
      ...
      <form method="POST" name="quizform" id="quizform" action="<?php echo base_url();?>admin/addeditQuiz">
         ...
         <div class="form-group">
            <label for="inputEmail">Quiz Name</label>
            <?php 
               $val = '';
         ...
         <div class="form-group">
            <label for="inputEmail">Negative Marks Status</label>
            <?php 
               $val = 'Inactive';
         ...
         <div class="form-group" id="NegativeMarkDiv" <?php if(count($data)) { if($data->negativemarkstatus=="Inactive") echo "style='display:none;'"; } else echo "style='display:none;'"; ?>>
            <label for="inputEmail">Negative Mark</label>
            <?php 
               $val = '';
         ...
         <div class="form-group">
            ...
            <input type="number" class="form-control" id="deauration" name="deauration" value="<?php echo $val;?>" placeholder="Enter Duration for Quiz (in minutes; if quiz duration is one hour, type as 60.)" >
			 ...
			 <div class="form-group">
            <label for="inputEmail">Quiz Type</label>
            <?php 
               $val = 'Free';
		 ...
		 <?php if(count($quizfor)>0) { ?>
		   ...
		   <div class="form-group">
		    <label for="inputEmail">Quiz For</label>&nbsp;&nbsp;&nbsp;
  		 <label>
          <input type="checkbox" id="for_all" name="for_all" <?php if(count($data)) { if($data->quiz_for == "0") echo "checked";}?>> All Users
        </label>
        <br>
        <label>
         ...
         <div class="form-group" id="quiztypediv" <?php if(count($data)) { if($data->quiztype=="Free") echo "style='display:none;'"; } else echo "style='display:none;'"; ?>>
		  ...
		  <label for="inputEmail">Validity Type</label>
		    <?php 
               $val = 'Days';
            ...
            <label for="inputEmail">Validity Value</label>
            <?php 
               $val = '0';
         ...
         <h4 id="hfour">Set Quiz Questions by selecting subjects</h4>
         <div class="col-md-12">
            <div class="col-md-6">
               <div class="form-group">
            ...
            <div class="col-md-6">
               <div class="form-group">
                  <label for="inputEmail">No. of Questions</label>
         ...
         <button type="button" style="float:right;" class="btn btn-primary wnm-user" id="quiz_questions">Add</button>
         ...
         <button type="submit" id="submitbtn" class="btn btn-primary wnm-user"><?php if(count($data)>0) echo "Update"; else echo "Save";?></button>
      </form>
   </div>
</div>
<!-- Validations -->
          ...
          {				
   		...
   		//form validation rules
              $("#quizform").validate({
   			...
   			messages: {
   				name: {
                          required: "Please enter title for quiz."
                      },
   				catid: {
   				...
   				startdate: {
   					required: "Please enter start date of quiz."
   				},
   				enddate: {
   					required: "Please enter end date of quiz."
   				},
   				deauration: {
   					required: "Please enter duration for quiz in minutes."
   				},
   				qq: {
