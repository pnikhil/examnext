...
*/
		...
		if ($this->form_validation->run() == true) {
			...
			if ($this->input->post('id') == '' ) {
				...
				$this->db->dbprefix('categories'), $where)){
					...
					$config['allowed_types'] = 'gif|jpg|png';
					$config['overwrite'] = TRUE;
					$config['file_name'] = "category_".$this->input->post('id');
					
					$this->load->library('upload', $config);
					
					if ($this->upload->do_upload())
					{ 
						$inputdata1['image'] = base_url()."assets/uploads/category_images/category_".$this->input->post('id').".".$f_type[$last_in];
	...
	function subcategories()
	{
		...
		if ($this->uri->segment(3) != '' && is_numeric($this->uri->segment(3))) {
			...
			$this->db->select('image');
			$rec = $this->db->get_where('subcategories',array("subcatid"=>$this->uri->segment(3)))->result()[0];
			unlink('./assets/uploads/sub_category_images/'.$rec->image);
			$this->base_model->delete_record(
	...
	function addeditSubCategories()
	{
		...
		if ($this->form_validation->run() == true) {
			...
			if ($this->input->post('id') == '' ) {
				...
				if($ref>0){
					$config['upload_path'] = './assets/uploads/sub_category_images';
					$config['allowed_types'] = 'gif|jpg|png';
					$config['overwrite'] = TRUE;
					$config['file_name'] = "subcategory_".$ref;
					
					$this->load->library('upload', $config);

					if ($this->upload->do_upload())
					{ 
						$inputdata1['image'] = base_url()."assets/uploads/sub_category_images/subcategory_".$ref.".".$f_type[$last_in];
						$where['subcatid'] = $ref;
					$this->base_model->update_operation($inputdata1, $this->db->dbprefix('subcategories'), $where);
					}
				...
				if($this->base_model->update_operation($inputdata, 
				$this->db->dbprefix('subcategories'), $where )){
					$config['upload_path'] = './assets/uploads/sub_category_images';
					$config['allowed_types'] = 'gif|jpg|png';
					$config['overwrite'] = TRUE;
					$config['file_name'] = "subcategory_".$this->input->post('id');
					
					$this->load->library('upload', $config);

					if ($this->upload->do_upload())
					{ 
						$inputdata1['image'] = base_url()."assets/uploads/sub_category_images/subcategory_".$this->input->post('id').".".$f_type[$last_in];
						$where['subcatid'] = $this->input->post('id');
					$this->base_model->update_operation($inputdata1, $this->db->dbprefix('subcategories'), $where);
					}
		...
		$this->db->dbprefix('categories'), array('status' => 'Active')
		);
	...
	function addeditQuestions()
	{
		...
		$this->form_validation->set_rules('ansdesc', 'Explaination', 'trim|required');
		$this->form_validation->set_rules(
		'correctanswer', 
		'Correct Answer', 
		'trim|required'
		...
		if ($this->input->post()) {
			if ($this->form_validation->run() == true) {
				...
				$inputdata['hint'] 			= $this->input->post('ansdesc');;
...
//		$ans['5'] 						= '5';
		...
		$this->db->dbprefix('subjects'), array('status' => 'Active')
		);
	...
	function settings()
	{
		...
		if ($this->form_validation->run() == true) {
			...
			$inputdata['site_timezone'] 	= $this->input->post('site_timezone');
			...
			$inputdata['contact_us'] 		= $this->input->post('contact_us');
			$inputdata['powered_by'] 		= $this->input->post('powered_by');
			$inputdata['url_for_powered_by']= $this->input->post('url_for_powered_by');
			$this->base_model->update_operation(
			$inputdata, 
		...
		//TimeZone Options
		$time_zones = $this->base_model->fetch_records_from('calendar|timezones');
	...
	public function aboutUs($param = '', $param1 = '')
	{
		...
		if ($param == "Add") {
			if ($this->input->post('submit') == "Add") {
				...
				if ($this->form_validation->run() == TRUE) {
					...
					if ($this->base_model->insert_operation($inputdata, $table_name)) {
						$this->prepare_flashmessage("Page Added Successfully" , 0);
						redirect('admin/aboutUs', 'refresh');
					}
					else {
						$this->prepare_flashmessage("Unable to Add Page" , 1);
			...
			$categories = $this->db->get_where('pages', array('parent_id' => 0))->result();
		...
		elseif ($param == "Edit") {
			...
			if ($this->input->post('submit') == "Update") {
				...
				if ($this->form_validation->run() == TRUE) {
					...
					if ($this->base_model->update_operation($inputdata, $table_name, $where)) {
						...
						$this->prepare_flashmessage("Unable to Upload Page" , 1);
						redirect('admin/aboutUs');
					}
				}
			...
			foreach($categories as $row) $category_opts[$row->id] = $row->name;
			$this->data['category_opts'] 		= $category_opts;
			$aboutus_rec = $this->base_model->run_query("SELECT * FROM " . $this->db->dbprefix('pages') . " WHERE id = " . $param1);
			$this->data['aboutus_rec'] 			= $aboutus_rec[0];
			// echo "<pre>"; print_r($aboutus_rec); die();
		...
		elseif ($param == "Delete") {
			...
			if ($this->base_model->check_duplicate($table_name, $cond, $cond_val) && is_numeric($param1)) {
				if ($this->base_model->delete_record($table_name, $where)) {
					$this->prepare_flashmessage("Page Deleted Successfully" , 0);
					redirect('admin/aboutUs', 'refresh');
				}
				else {
					$this->prepare_flashmessage("Unable to Delete Page" , 1);
					redirect('admin/aboutUs');
				}
			}
			else {
		...
		elseif ($param1 == "") {
			$aboutus_recs = $this->base_model->run_query("SELECT * FROM " . $this->db->dbprefix('pages'));
	...
	public function socialNetworks($param = '', $param1 = '')
	{
		...
		// echo "<pre>"; print_r($network_settings); die();
