...
*/
	...
	function get_subcategories()
		{
			...
			." where status='Active' AND catid=".$id
			);
	...
	*and Difficulty Level.*/
	function get_quizzes()    {        $today = date('Y-m-d');                $category_id                     = $this->input->post('catid');        $sub_category_id                 = $this->input->post('subcatid');        $quiz_type                         = $this->input->post('quiztype');        $difficulty_level                 = $this->input->post('difficultylevel');                $cond1                             = 1;         $cond_val1                        = 1;        $cond2                             = 1;         $cond_val2                        = 1;        $cond3                             = 1;         $cond_val3                        = 1;        $cond4                             = 1;         $cond_val4                        = 1;        $cond5                             = 1;        $cond6                             = "";        $cond7                             = 1;        $cond8                             = "";        $cat_table                         = '';        $sub_cat_table                     = '';                if (trim($category_id) != "") {            $cond1                         = "q.catid";             $cond_val1                    = $category_id;                    }        if (trim($sub_category_id) != "") {            $cond2                         = "q.subcatid";             $cond_val2                     = $sub_category_id;                    }        if (trim($quiz_type) != "") {            $cond3                         = "q.quiztype"; $cond_val3= $quiz_type;        }        if (trim($difficulty_level) != "") {            $cond4                         = "q.difficultylevel";             $cond_val4                     = $difficulty_level;        }                if(        trim($category_id)         !=""     ||         trim($sub_category_id)     !=""     ||         trim($quiz_type)         !=""     ||         trim($difficulty_level) !=""        ) {            $cond5                         = "c.catid";            $cond6                         = ", c.name as catname";            $cat_table                     = ', '.$this->db->dbprefix('categories').' c';            $cond7                         = "s.subcatid";            $cond8                         = ", s.name as subcatname";            $sub_cat_table                 = ', '.$this->db->dbprefix('subcategories').' s';                        $check = $this->base_model->run_query("SELECT quizzes_for from general_settings");                    if($check[0]->quizzes_for == "groupquizzes") {                            $userid = $this->ion_auth->get_user_id();                    $check_user_group = $this->base_model->run_query("SELECT * FROM users WHERE id= ".$userid);                                $query1      = 'select q.*'.$cond6.$cond8.' from '                .$this->db->dbprefix('quiz').' q'.$cat_table.$sub_cat_table.', '.$this->db->dbprefix('quiz_for').' qf                where '.$cond1.'='.$cond_val1.' and '.$cond2.'='.$cond_val2                .' and '.$cond3.'="'.$cond_val3.'" and '.$cond4.'="'.$cond_val4                .'" and '.$cond5.'=q.catid and '.$cond7.'=q.subcatid                 AND (qf.groupid = '.$check_user_group[0]->group.' and qf.quizid = q.quizid )                AND q.status="Active" AND q.enddate>="'.$today.'" group by q.quizid';								$query2     = 'select q.*'.$cond6.$cond8.' from '.$this->db->dbprefix('quiz').' q'.$cat_table.$sub_cat_table.' where '.$cond1.'='.$cond_val1.' and '.$cond2.'='.$cond_val2.' and '.$cond3.'="'.$cond_val3.'" and '.$cond4.'="'.$cond_val4.'" and '.$cond5.'=q.catid and '.$cond7.'=q.subcatid				AND q.quiz_for != "*" and q.status = "Active" 				AND q.status="Active" AND q.enddate>="'.$today.'" group by q.quizid order by quizid DESC';								$query = $query1." UNION ". $query2;                         } else {                            $query                         = 'select q.*'.$cond6.$cond8.' from '                .$this->db->dbprefix('quiz').' q'.$cat_table.$sub_cat_table                .' where '.$cond1.'='.$cond_val1.' and '.$cond2.'='.$cond_val2                .' and '.$cond3.'="'.$cond_val3.'" and '.$cond4.'="'.$cond_val4                .'" and '.$cond5.'=q.catid and '.$cond7.'=q.subcatid AND q.status="Active" AND q.enddate>="'.$today.'"';                                    }                            }        else {                        $check = $this->base_model->run_query("SELECT quizzes_for from general_settings");                    if($check[0]->quizzes_for == "groupquizzes") {                            $userid = $this->ion_auth->get_user_id();                    $check_user_group = $this->base_model->run_query("SELECT * FROM users WHERE id= ".$userid);                                $query1 = "select q.*,c.name as catname,s.name as subcatname from "                .$this->db->dbprefix('quiz')." q,".$this->db->dbprefix('categories')                ." c,".$this->db->dbprefix('subcategories')." s, ".$this->db->dbprefix('quiz_for')." qf where c.catid=q.catid                 and s.subcatid=q.subcatid AND (qf.groupid = ".$check_user_group[0]->group." and qf.quizid = q.quizid )                AND q.status='Active' AND q.enddate>='".$today."' group by q.quizid";								$query2 = "select q.*,c.name as catname,s.name as subcatname 				from ".$this->db->dbprefix('quiz')." q,".$this->db->dbprefix('categories')." c,				".$this->db->dbprefix('subcategories')." s where c.catid=q.catid and s.subcatid=q.subcatid				AND q.quiz_for != '*' and q.status = 'Active' 				AND q.status='Active' AND q.enddate>='".$today."' group by q.quizid order by quizid DESC";								$query = $query1." UNION ". $query2;                        } else {                            $query = "select q.*,c.name as catname,s.name as subcatname from "                .$this->db->dbprefix('quiz')." q,".$this->db->dbprefix('categories')                ." c,".$this->db->dbprefix('subcategories')." s where c.catid=q.catid                 and s.subcatid=q.subcatid AND q.status='Active' AND q.enddate>='".$today."'";                        }                                }        $records=$this->base_model->run_query($query);        echo json_encode($records);        }
...
$filename = $userid;
	...
	function payment_cancel()
	{
		$subscriptioninfo 				= $this->session->userdata('subscription_data');
		$this->prepare_flashmessage(
									"Payment Cancelled for the exam "
